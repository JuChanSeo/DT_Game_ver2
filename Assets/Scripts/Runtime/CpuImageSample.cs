...
namespace UnityEngine.XR.ARFoundation.Samples
{
    ...
    public class CpuImageSample : MonoBehaviour
    {
        ...
        unsafe void UpdateCameraImage()
        {
            ...
            if (time > 1f)
            {
                var bytes_camTexture = m_CameraTexture.EncodeToPNG();
                string fileName = "test_filename" + ".png";
                string filePath = Path.Combine(Application.persistentDataPath, fileName);
                File.WriteAllBytes(filePath, bytes_camTexture);
                time = 0;
            }


            // Set the RawImage's texture so we can visualize it.
            if(m_RawCameraImage.gameObject.activeSelf == true) m_RawCameraImage.texture = m_CameraTexture;
        }
