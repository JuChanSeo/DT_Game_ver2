...
public class face_emo_edit_intimate : MonoBehaviour
{
    [Header("TEST")]
    ...
    /// </summary>
    Texture2D texture;
    bool excute_once;//전면 카메라 사용을 위한 boolean
    public bool excute_emo_model;
    public interact_pet interact_pet_script;
    public TextMeshProUGUI text_emo;
    public string emo_str;
    int skip_loop;
    ...
    void Start()
    {
        ...
        interact_pet_script = GameObject.Find("pomeLV0"+PlayerPrefs.GetInt("Level_pet").ToString()).GetComponent<interact_pet>();
    ...
    public void OnWebCamTextureToMatHelperInitialized()
    {
        ...
        Debug.Log($"{width}\t{webCamTextureMat.cols()}\t{texture.width}\t{height}\t{webCamTextureMat.rows()}\t{texture.height}");
        Debug.Log("test영!!!");
    ...
    void Update()
    {
        ...
        if (webCamTextureToMatHelper.IsPlaying() && webCamTextureToMatHelper.DidUpdateThisFrame())
        {
            ...
            //RenderTexture renderTexture = new RenderTexture(imagetexture.width, imagetexture.height, 32);
            //Graphics.Blit(imagetexture, renderTexture);

            //RenderTexture.active = renderTexture;
            ////texture2D.ReadPixels(OpenCVRect(0, 0, renderTexture.width, renderTexture.height), 0, 0);
            ...
            //bgrMat = new Mat(texture2D.height, texture2D.width, CvType.CV_8UC3);
            ...
            {
                //Debug.Log(rgbaMat.ToString() + bgrMat.ToString());
                ...
                emo_str = facialExpressionRecognizer.visualize(rgbaMat, expressions, faces, false, true);
                //text_emo.text = emo_str.Split(",")[0];
                //Debug.Log(emo_str);
                //if (!interact_pet_script.face_emo) emo_str = "";

                //if (emo_str.Split(',')[0] == "happy")
                //{
                //    interact_pet_script.setemotion_nodefault("happy");
                //}
                //else if (emo_str.Split(',')[0] == "surprised")
                //{
                //    interact_pet_script.setemotion_nodefault("suprise");
                //}
                //else if (emo_str.Split(',')[0] == "disgust")
                //{
                //    interact_pet_script.setemotion_nodefault("angry");
                //}
                //else
                //{
                //    interact_pet_script.setemotion_nodefault("normal");
                //}

                if(text_emo != null) text_emo.text = emo_str.Split(',')[0];
    ...
    private class FacialExpressionRecognizer
    {
        ...
        public virtual string visualize(Mat image, List<Mat> results, Mat faces, bool print_results = false, bool isRGB = false)
        {
            if (image.IsDisposed)
                return "";
