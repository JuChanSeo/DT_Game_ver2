...
public class InferenceController_edit : MonoBehaviour
{
    #region Fields
    ...
    public RawImage face_texture;
    public TMPro.TextMeshProUGUI text_ges;
    ...
    private void Update()
    {
        ...
        if (!excute_ges_recog) return;
        ////Debug.Log("check2");
        //// Get the input image and dimensions
        //var imageTexture = screenRenderer.material.mainTexture;
        ////var imageTexture = screenRenderer.material.mainTexture as Texture2D;
        ////Debug.Log("check2-1");
        ...
        // Get the input image and dimensions
         ...
         //var imageTexture = screenRenderer.material.mainTexture;
         var imageTexture = face_texture.mainTexture;
        //Debug.Log("check2-1");
        //Debug.Log(imageTexture.width + "\t" + imageTexture.height);
        var imageDims = new Vector2Int(imageTexture.width, imageTexture.height);
        //Debug.Log(imageDims);
    ...
    private void UpdateBoundingBoxes(Vector2Int inputDims)
    {
        ...
        for (int i = 0; i < bboxInfoArray.Length; i++)
        {
          ...
          //+ "\t" + bboxInfoArray[i].bbox.width + "\t" + bboxInfoArray[i].bbox.height);
            text_ges.text = bboxInfoArray[i].label;
