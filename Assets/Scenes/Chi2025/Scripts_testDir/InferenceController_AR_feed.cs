...
public class InferenceController_AR_feed : MonoBehaviour
{
    #region Fields
    ...
    public void game_start_button_click()
    {
        time_remain = 30f;
        text_time.text = "";
        //게임이 처음 시작되었으면 바로 listening 시작
        //게임이 이미 시작했을 경우 5초뒤에 listening 시작
        if (!start_flag) start_listening();
        else Invoke("start_listening", 5f);
    ...
    public void start_listening()
    {
        ...
        time_remain_text = 4f;
        Invoke("stop_listening", 4f);
        text_instruct.text = list_instruct[idx_answer];
    ...
    void check_answer()
    {
        /*
         { "강아지를 불러주세요",
          ...
          "강아지가 애교부리게 해주세요"};
         */
        if (idx_answer == 0)//강아지를 불러주세요 
        {
            if (text_STT.text == "이리 와")
            {
                cnt_succes += 1;
                text_succes.text = "성공: " + cnt_succes.ToString();
                anim.Play("311_Stroll_Wag");
            }
            else //지시문 실패한 경우
            {
                cnt_fail += 1;
                text_fail.text = "실패: " + cnt_succes.ToString();
            }
        }
        else if (idx_answer == 1)//강아지가 한바퀴 돌게 해주세요
        {
            if (text_STT.text == "돌아")
            {
                cnt_succes += 1;
            ...
            {
                cnt_fail += 1;
                text_fail.text = "실패: " + cnt_fail.ToString();
            }
        }
        else if (idx_answer == 2)//강아지가 제자리에서 점프하게 해주세요
        {
            if (text_STT.text == "뛰어")
            {
                cnt_succes += 1;
                text_succes.text = "성공: " + cnt_fail.ToString();
                anim.Play("Jump");
            }
            else //지시문 실패한 경우
            {
        ...
        else if (idx_answer == 3)//강아지가 눕게 해주세요
        {
            if (text_STT.text == "누워")
            {
                cnt_succes += 1;
        ...
        else if (idx_answer == 4)//강아지가 애교부리게 해주세요
        {
            if (text_STT.text == "애교")
            {
                cnt_succes += 1;
